...
export default async function CourseDetailPage({ params }: Props) {
  ...
  const tCourses = await getTranslations('coursesPage')
  ...
  const levelLabels: Record<string, string> = {
    BEGINNER: tCourses('filterBeginner'),
    INTERMEDIATE: tCourses('filterIntermediate'),
    ADVANCED: tCourses('filterAdvanced'),
  }
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-12 pt-8 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap items-center gap-6 text-sm text-gray-400">
            ...
            <span className="flex items-center gap-2">
              ...
              {t('lessonCount', { count: course.lessons.length })}
            </span>
            <span className="flex items-center gap-2">
            ...
            <span className="flex items-center gap-2">
              ...
              {t('studentCount', { count: course._count.enrollments.toLocaleString() })}
            </span>
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {course.instructor && (
              <div>
                <h2 className="mb-5 text-xl font-bold text-white">{t('instructor')}</h2>
                ...
                <div className="p-6">
                  <div className="mb-4 text-center">
                    <p className="text-3xl font-bold text-green-400">
                      {course.isFree ? t('free') : t('premium')}
                    </p>
                    {course.isFree && (
                  ...
                  <div className="mt-6 space-y-3 border-t border-white/[0.06] pt-6">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-400">{t('totalLessons')}</span>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium text-green-400">
                        {course.hasCertificate ? t('hasCert') : t('noCert')}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-400">{t('studentsEnrolled')}</span>
                      <span className="font-medium text-gray-200">
                        {t('studentCount', { count: course._count.enrollments.toLocaleString() })}
                      </span>
                    </div>
