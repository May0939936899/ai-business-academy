...
function getGreetingKey(): { greetingKey: string; messageKey: string } {
  const hour = new Date().getHours()
  if (hour < 12)
    return { greetingKey: 'greetingMorning', messageKey: 'messageMorning' }
  if (hour < 17)
    return { greetingKey: 'greetingAfternoon', messageKey: 'messageAfternoon' }
  return { greetingKey: 'greetingEvening', messageKey: 'messageEvening' }
}
...
export default async function DashboardPage() {
  ...
  const { greetingKey, messageKey } = getGreetingKey()
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="mb-10 flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-5">
            ...
            <div>
              <p className="text-sm font-medium text-blue-400">{t(greetingKey)}</p>
              <h1 className="text-2xl font-bold text-white sm:text-3xl">
                {user.fullName || user.name || t('learner')}
              </h1>
              <p className="mt-1 max-w-lg text-sm text-gray-400">{t(messageKey)}</p>
            </div>
          </div>
        ...
        <section className="mb-12">
          <div className="mb-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-white">{t('continueLearningSection')}</h2>
            </div>
            <Link
              href={`/${locale}/courses`}
                      ...
                      <div className="mb-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500">
                        ...
                        {enrollment.lastViewedAt && (
                          <span className="flex items-center gap-1">
                            ...
                            {t('lastViewed', { date: formatDate(enrollment.lastViewedAt, 'd MMM yyyy') })}
                          </span>
                        )}
                      </div>
                      ...
                      <Link
                        href={
                          enrollment.nextLessonId
                            ? `/${locale}/learn/${enrollment.course.slug}/${enrollment.nextLessonId}`
                            : `/${locale}/courses/${enrollment.course.slug}`
                        }
                        className={cn(
        ...
        {completedCourses.length > 0 && (
          <section className="mb-12">
            <div className="mb-6 flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-white">{t('completedCourses')}</h2>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1.5 flex items-center gap-1.5 text-xs text-gray-500">
                          ...
                          {t('completedAllLessons', { count: enrollment.totalLessons })}
                        </p>
                        {enrollment.completedAt && (
                          <p className="mt-0.5 flex items-center gap-1.5 text-xs text-gray-500">
                            <Clock className="h-3.5 w-3.5" />
                            {t('completedOn', { date: formatDate(enrollment.completedAt, 'd MMM yyyy') })}
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    {cert && (
                      <div className="mt-4">
                        <Link
                          href={`/${locale}/certificate/${cert.certificateCode}`}
                          className={cn(
        ...
        <section className="mb-12">
          <div className="mb-6 flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-white">{t('myCertificates')}</h2>
          </div>
              ...
              {certificates.map((cert) => (
                ...
                >
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 flex items-center gap-1.5 text-xs text-gray-500">
                        <Clock className="h-3.5 w-3.5" />
                        {t('issuedOn', { date: formatDate(cert.issuedAt, 'd MMMM yyyy') })}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 flex gap-2">
                    ...
                    <Link
                      href={`/${locale}/a/certificate/${cert.certificateCode}`}
                      className={cn(
                        'flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl px-3.5 py-2 text-xs font-semibold',
          ...
          <section>
            <div className="mb-6 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold text-white">{t('recommendedCourses')}</h2>
              </div>
              <Link
                href={`/${locale}/courses`}
                className="flex items-center gap-1 text-sm font-medium text-blue-400 transition-colors hover:text-blue-300"
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-3 flex items-center gap-3 text-[11px] text-gray-500">
                      ...
                      <span className="flex items-center gap-1">
                        ...
                        {course._count.enrollments} {t('people')}
                      </span>
                    </div>
                  </div>
                </Link>
